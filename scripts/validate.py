...
class Page(HTMLParser):
 def __init__(self,text):
  super().__init__();self.links=[];self.ids=set();self.h1=0;self.canonical=[];self.desc=[];self.images=[];self.feed(text)
...
pages=[p for p in ROOT.rglob('*.html') if 'templates' not in p.parts and 'docs' not in p.parts]
...
for i in icons:
 source=ROOT/f'icons/{i["slug"]}.svg';tree=ET.fromstring(source.read_text())
 ...
 path=ROOT/f'icons/{i["slug"]}/index.html';text=path.read_text()
